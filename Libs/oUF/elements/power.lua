...
local function GetColorByKey(colors, key)
	if(not colors or key == nil or (issecretvalue and issecretvalue(key))) then return end

	local ok, color = pcall(function() return colors[key] end)
...
local function UpdateColor(self, event, unit)
	...
	elseif(element.colorThreat and not UnitPlayerControlled(unit) and UnitThreatSituation('player', unit)) then
		color = GetColorByKey(self.colors.threat, UnitThreatSituation('player', unit))
	elseif(element.colorPower) then
		if(element.displayType) then
			color = GetColorByKey(self.colors.power, element.displayType)
		end

		if(not color) then
			local pType, pToken, altR, altG, altB = UnitPowerType(unit)
			color = GetColorByKey(self.colors.power, pToken)
			...
			if(not color and altR) then
				...
				color = GetColorByKey(self.colors.power, pType) or self.colors.power.MANA
			end
		end
	...
	elseif(element.colorClass and (UnitIsPlayer(unit) or UnitInPartyIsAI(unit)))
		...
		color = GetColorByKey(self.colors.class, class)
	elseif(element.colorSelection and unitSelectionType(unit, element.considerSelectionInCombatHostile)) then
		color = GetColorByKey(self.colors.selection, unitSelectionType(unit, element.considerSelectionInCombatHostile))
	elseif(element.colorReaction and UnitReaction(unit, 'player')) then
		color = GetColorByKey(self.colors.reaction, UnitReaction(unit, 'player'))
	end
