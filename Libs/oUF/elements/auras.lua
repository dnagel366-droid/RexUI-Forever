...
local function HideAuraElement(element)
	...
	element.visibleAuras = 0
	element.visibleBuffs = 0
	element.visibleDebuffs = 0
end

local function UnitAurasAreSecret(unit)
...
local UpdateAuras
local ScheduleSecretAuraRefresh

local function MarkAuraElementsNeedFullUpdate(self)
	if(self.Auras) then self.Auras.needFullUpdate = true end
	if(self.Buffs) then self.Buffs.needFullUpdate = true end
	if(self.Debuffs) then self.Debuffs.needFullUpdate = true end
end

local function HideSecretAuras(self)
	MarkAuraElementsNeedFullUpdate(self)
	HideAuraElement(self.Auras)
	HideAuraElement(self.Buffs)
	HideAuraElement(self.Debuffs)
	if(ScheduleSecretAuraRefresh) then
		ScheduleSecretAuraRefresh(self)
	end
end

local function SafeGetAuraSlots(unit, filter)
	local slots = {pcall(C_UnitAuras.GetAuraSlots, unit, filter)}
	if(not slots[1]) then
		return nil
	end
...
ScheduleSecretAuraRefresh = function(self)
	if(not C_Timer or not self.unit) then return end

	self.__secretAuraRefreshToken = (self.__secretAuraRefreshToken or 0) + 1
	local token = self.__secretAuraRefreshToken
	local unit = self.unit

	C_Timer.After(0.25, function()
		if(token ~= self.__secretAuraRefreshToken or self.unit ~= unit) then
			return
		end

		if(UnitAurasAreSecret(unit)) then
			return
		end

		UpdateAuras(self, 'ForceUpdate', unit)
	end)
end

function UpdateAuras(self, event, unit, updateInfo)
	if(self.unit ~= unit) then return end
	if(UnitAurasAreSecret(unit)) then
		HideSecretAuras(self)
		return
	end

	if(updateInfo) then
		if(not CanAccess(updateInfo.isFullUpdate)
		or not CanAccess(updateInfo.addedAuras)
		or not CanAccess(updateInfo.updatedAuraInstanceIDs)
		or not CanAccess(updateInfo.removedAuraInstanceIDs)) then
			HideSecretAuras(self)
			return
		end
	end

	local isFullUpdate = not updateInfo or updateInfo.isFullUpdate == true
	...
	if(auras) then
		...
		* isFullUpdate - indicates whether the element is performing a full update (boolean)
		...
		if(isFullUpdate) then
			...
			if(not slots) then HideSecretAuras(self) return end
