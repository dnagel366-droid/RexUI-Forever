...
local MINOR_VERSION = 88 -- the real minor version is 155
...
-- Forever RestrictedExecution.lua:79 calls loadstring_untainted, which is nil.
-- Skip header:Execute; clicks, attributes, and paging stay on the insecure/state path.
local CAN_EXECUTE_RESTRICTED = type(loadstring_untainted) == "function"
...
-- Forever RestrictedExecution has no newtable(), GetFrameRef(), or RunFor().
-- Custom LAB flyouts Execute("LAB_FlyoutInfo = newtable()") and crash at
-- RestrictedExecution.lua:79. Use Blizzard SpellFlyout instead (Ellesmere).
local UseCustomFlyout = CAN_EXECUTE_RESTRICTED and FlyoutButtonMixin and not ActionButton_UpdateFlyout
...
function lib:CreateButton(id, name, header, config)
	...
	button:HookScript("OnMouseUp", function(self)
		self._labDragHandled = nil
		self._labDidPickup = nil
	end)
...
function SetupSecureSnippets(button)
	...
	if CAN_EXECUTE_RESTRICTED then
		...
		-- Forever cannot compile WrapScript. PickupAction/PlaceAction stay insecure.
		button:SetScript("OnDragStart", function(self)
			if InCombatLockdown() or self:GetAttribute("LABdisableDragNDrop") then return end
			local locked = self:GetAttribute("buttonlock")
			local pickup = (IsModifiedClick and IsModifiedClick("PICKUPACTION")) or (IsShiftKeyDown and IsShiftKeyDown())
			if locked and not pickup then return end
			local action = self:GetAttribute("action")
			if action then
				pcall(PickupAction, action)
			end
		end)
...
function WrapOnClick(button, unwrapheader)
	...
	-- Forever has no loadstring_untainted; skip compiled click wrappers.
	-- SecureActionButton type/action clicks remain active.
	if not CAN_EXECUTE_RESTRICTED then
		return
	end
...
function Generic:UpdateState(state)
	...
	-- Forever RestrictedExecution has no GetFrameRef/RunFor. Mirror the
	-- UpdateState snippet from insecure Lua. Combat paging still uses
	-- _childupdate-state -> RunAttribute("UpdateState").
	self:SetAttribute("state", state)
	...
	if kind == "action" and action then
		local _, hasAction = GetActionInfo(action)
...
if UseCustomFlyout then
	...
	local FlyoutHandleFunc = [[
		...
		local flyoutID = ...
		local numSlots = self:GetAttribute("LABFlyoutSlots" .. flyoutID)
		...
		for slotID = 1, numSlots do
			if self:GetAttribute("LABFlyoutKnown" .. flyoutID .. "-" .. slotID) then
				local spellID = self:GetAttribute("LABFlyoutSpell" .. flyoutID .. "-" .. slotID)
				local overrideSpellID = self:GetAttribute("LABFlyoutOverride" .. flyoutID .. "-" .. slotID)
				local spellName = self:GetAttribute("LABFlyoutName" .. flyoutID .. "-" .. slotID)
				...
				slotButton:SetAttribute("spell", spellID)

				-- custom ones for elvui
				slotButton:SetAttribute("spellName", spellName)

				-- set LAB attributes
				slotButton:SetAttribute("labtype-0", "spell")
				slotButton:SetAttribute("labaction-0", spellID)

				-- run LAB updates
				slotButton:CallMethod("SetStateFromHandlerInsecure", 0, "spell", overrideSpellID or spellID)
	...
	local function SyncFlyoutInfoToHandler()
		...
		local handler = GetFlyoutHandler()
		for flyoutID in pairs(syncedFlyoutIDs) do
			handler:SetAttribute("LABFlyoutSlots" .. flyoutID, nil)
		end
		syncedFlyoutIDs = {}
		for flyoutID, info in pairs(lib.FlyoutInfo) do
			if info.isKnown then
				...
				for slotID, slotInfo in ipairs(info.slots) do
					local suffix = flyoutID .. "-" .. slotID
					handler:SetAttribute("LABFlyoutSpell" .. suffix, slotInfo.spellID)
					handler:SetAttribute("LABFlyoutOverride" .. suffix, slotInfo.overrideSpellID)
					handler:SetAttribute("LABFlyoutKnown" .. suffix, slotInfo.isKnown or nil)
					handler:SetAttribute("LABFlyoutName" .. suffix, slotInfo.spellName)
					numSlots = numSlots + 1
				end
				handler:SetAttribute("LABFlyoutSlots" .. flyoutID, numSlots)
				syncedFlyoutIDs[flyoutID] = true
...
function Generic:PreClick()
	if InCombatLockdown() or self:GetAttribute("LABdisableDragNDrop") then
		return
	end

	if not CAN_EXECUTE_RESTRICTED then
		-- Only suppress the secure cast when a pickup/place is actually in
		-- progress. A leftover flag must not eat normal Bar 1-8 clicks.
		if (self._labDragHandled or self._rexPickupDown) and GetCursorInfo() then
			self:SetAttribute("type", nil)
			return
		end
		self._labDragHandled = nil
		self._rexPickupDown = nil
		local action = self:GetAttribute("action") or self._state_action
		if (self._state_type == "action" or self._state_type == "pet") and action then
			local kind = GetCursorInfo()
			local pickup = (IsModifiedClick and IsModifiedClick("PICKUPACTION")) or (IsShiftKeyDown and IsShiftKeyDown())
			if pickup and not kind then
				self._old_type = self:GetAttribute("type")
				self:SetAttribute("type", nil)
				if self._state_type == "pet" and PickupPetAction then
					pcall(PickupPetAction, action)
				else
					pcall(PickupAction, action)
				end
				self._labDidPickup = true
				self._labDragHandled = true
				return
			end
			if kind then
				self._old_type = self:GetAttribute("type")
	...
	if self._state_type == "action" or self._state_type == "pet" then
		return
	end
...
function Generic:PostClick(button, down)
	...
	if self._old_type and not InCombatLockdown() then
		self:SetAttribute("type", self._old_type)
		self._old_type = nil
	end

	if self._labDidPickup then
		self._labDidPickup = nil
	elseif self._receiving_drag and not InCombatLockdown() then
		...
		if not CAN_EXECUTE_RESTRICTED then
			local action = self:GetAttribute("action") or self._state_action
			if action then
				if oldType == "pet" and PickupPetAction then
					pcall(PickupPetAction, action)
				else
					pcall(PlaceAction, action)
				end
...
local function UpdateTextElement(button, element, config, defaultFont, fromRange)
	local rangeIndicator = fromRange and element:GetText() == _G.RANGE_INDICATOR
...
local function UpdateTextElements(button)
	UpdateTextElement(button, button.HotKey, button.config.text.hotkey, (_G.NumberFontNormalSmallGray:GetFont()))
	UpdateTextElement(button, button.Count, button.config.text.count, (_G.NumberFontNormal:GetFont()))
	UpdateTextElement(button, button.Name, button.config.text.macro, (_G.GameFontHighlightSmallOutline:GetFont()))
end
...
function Generic:OnUpdate(elapsed)
	if self.flashing then
		...
		if self.flashTime <= 0 then
			...
			self.flashTime = self.flashTime + _G.ATTACK_BUTTON_FLASH_TIME
		end
	end
	...
	if not WoWRetail then
		...
		if self.rangeTimer <= 0 then
			...
			self.rangeTimer = _G.TOOLTIP_UPDATE_TIME
		end
	end
end
...
function UpdateRange(button, force, inRange, checksRange) -- Sezz: moved from OnUpdate
	...
	if force or (oldRange ~= button.outOfRange) then
		...
		elseif button.config.outOfRangeColoring == "hotkey" and not button.config.hideElements.hotkey then
			UpdateTextElement(button, button.HotKey, button.config.text.hotkey, _G.NumberFontNormalSmallGray:GetFont(), true)
		end
...
function Update(self, which)
	...
	if isTypeAction and not InCombatLockdown() then
		...
		if updateReleaseCasting and CAN_EXECUTE_RESTRICTED then -- zone in dragon mount on Evokers can bug
			self.header:SetFrameRef("updateButton", self)
			self.header:Execute(([[
				local frame = self:GetFrameRef("updateButton")
		...
		local onStateChanged = self:GetAttribute("OnStateChanged")
		if onStateChanged and CAN_EXECUTE_RESTRICTED then
			self.header:SetFrameRef("updateButton", self)
			self.header:Execute(([[
				local frame = self:GetFrameRef("updateButton")
...
function UpdateHotkeys(self)
	...
	if not key or key == "" or self.config.hideElements.hotkey then
		self.HotKey:SetText(_G.RANGE_INDICATOR)
		self.HotKey:Hide()
	else
...
elseif FlyoutButtonMixin then -- Retail custom flyout, or Forever native SpellFlyout
	if UseCustomFlyout then
	...
	function UpdateFlyout(self)
		...
		if self._state_type == "action" then
			-- based on ActionButton_UpdateFlyout in ActionButton.lua
			local actionType = GetActionInfo(self._state_action)
			if actionType == "flyout" and self.Arrow then
				self.Arrow:Show()
				if self.UpdateArrowTexture then
					self:UpdateArrowTexture()
				end
				if self.UpdateArrowRotation then
					self:UpdateArrowRotation()
				end
				if self.UpdateArrowPosition then
	...
	function UpdateFlyout(self, isButtonDownOverride)
		...
		if self._state_type == "action" then
			-- based on ActionButton_UpdateFlyout in ActionButton.lua
			local actionType = GetActionInfo(self._state_action)
			-- Forever ActionButtonTemplate has Arrow/BorderShadow, but no FlyoutArrowContainer.
			if actionType == "flyout" and self.FlyoutArrowContainer then
		...
		if hideArrow and self.FlyoutArrowContainer then
...
Item.GetCooldownInfo         = function(self) local start, duration, enable = C_Container_GetItemCooldown(getItemId(self._state_action)) return { startTime = start, duration = duration, isEnabled = enable, modRate = 1.0, isActive = (enable and start and start > 0 and duration and duration > 0) } end
...
if UseCustomFlyout and oldversion and lib.flyoutHandler then
	UpdateFlyoutHandlerScripts()
end
